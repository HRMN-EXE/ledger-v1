// ─── Kairos offline engine — same api() contract, zero network ──────────────
...
function createTask(db: LocalDB, body: Body): TaskDTO {
  ...
  const task: TaskDTO = {
    ...
    // should tasks always carry a compulsory, user-chosen deadline — never a default
    deadline: priority === 2 ? (typeof body?.deadline === "string" ? body.deadline : null) : null,
  };
  if (task.priority === 2 && !task.deadline) {
    throw new HttpError(400, "Should tasks require a deadline");
  }
  db.tasks.push(task);
  saveDB(db);
  return task;
...
function patchTask(db: LocalDB, id: number, body: Body): TaskDTO {
  ...
  if (body?.deadline !== undefined) {
    row.deadline = typeof body.deadline === "string" ? body.deadline : null;
  }
  if (body?.priority != null && sanitizePriority(body.priority) !== 2 && body?.deadline === undefined) {
    row.deadline = null;
  }
  if (row.priority === 2 && !row.deadline && (body?.priority != null || body?.deadline !== undefined)) {
    throw new HttpError(400, "Should tasks require a deadline");
  }
...
function createRitual(db: LocalDB, body: Body): { duplicate?: { id: number; name: string }; created?: number } {
  ...
  const rawTenure = Number(body?.tenureValue);
  if (!Number.isFinite(rawTenure) || rawTenure < 1) {
    throw new HttpError(400, "Tenure is required — pick how long the commitment runs");
  }
  const tenureValue = Math.min(3650, Math.max(1, Math.round(rawTenure)));
...
function ritualAction(db: LocalDB, ritualId: number, body: Body): void {
  ...
  if (action === "edit") {
    if (current?.status === "COMPLETE") throw new HttpError(403, "Completed rituals are sealed — read-only");
...
function deleteRitual(db: LocalDB, ritualId: number): void {
  if (db.commitments.some((c) => c.ritualId === ritualId && c.status === "COMPLETE")) {
    throw new HttpError(403, "Completed rituals are sealed — read-only");
  }
...
export async function api<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    if (method === "POST" && route === "/api/focus") {
      const durationMin = Math.max(1, num(body?.durationMin, 25));
      const session = {
        id: nextId(db),
        label: String(body?.label ?? "Focus").trim() || "Focus",
        durationMin,
        durationSec: Math.max(0, num(body?.durationSec, durationMin * 60)),
